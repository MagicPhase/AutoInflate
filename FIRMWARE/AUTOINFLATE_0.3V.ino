...
All variables including pressures, settings, and times are converted from the (encoderInput) counter. This has the effect of limting the divisions of certain
...
to acheieve the max value. The division of the PWM speed is dictated by the number of pixels in the display graph. This is handled via the map function to
...
While the displayed pressures are in PSI, the stored pressure vaules are in (mBar * 100). Example: The default stored value for the maximum "selectable" 
...
Inputing pressures over the selected maximum will reduce the value to the maximum. An inflation task "hug or profile" will only execute if the times are not zero!
...
items require a reset to take effect. This is forcued in the CONFIG navigation.
...
that can make the pump quieter during topping maintance.
Motor pulses for system feedback. (One) pulse for system functions such as save function or failure to set a variable. (Two) pulses single the end of an inflation task.
*/
